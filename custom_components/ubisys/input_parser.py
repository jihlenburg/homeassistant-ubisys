...
class InputAction:
    ...
    def to_friendly_string(self) -> str:
        """Return a user-friendly string description of the action.

        Used for "Dry Run" summaries in the UI.
        """
        press_desc = self.press_type.value.replace("_", " ").title()

        # Decode common clusters and commands
        command_desc = f"Command 0x{self.command_id:02X}"
        cluster_name = f"0x{self.cluster_id:04X}"

        if self.cluster_id == 0x0006:  # OnOff
            cluster_name = "OnOff"
            if self.command_id == 0x00:
                command_desc = "Turn Off"
            elif self.command_id == 0x01:
                command_desc = "Turn On"
            elif self.command_id == 0x02:
                command_desc = "Toggle"

        elif self.cluster_id == 0x0008:  # Level Control
            cluster_name = "Level"
            if self.command_id == 0x01:
                mode = (
                    "Up"
                    if self.command_payload and self.command_payload[0] == 0x00
                    else "Down"
                )
                command_desc = f"Move {mode}"
            elif self.command_id == 0x02:
                command_desc = "Step"
            elif self.command_id == 0x04:
                command_desc = "Stop"
            elif self.command_id == 0x05:
                mode = (
                    "Up"
                    if self.command_payload and self.command_payload[0] == 0x00
                    else "Down"
                )
                command_desc = f"Move {mode} (with On/Off)"

        elif self.cluster_id == 0x0102:  # Window Covering
            cluster_name = "Cover"
            if self.command_id == 0x00:
                command_desc = "Up/Open"
            elif self.command_id == 0x01:
                command_desc = "Down/Close"
            elif self.command_id == 0x02:
                command_desc = "Stop"

        # Format: "Input 1 (Short Press): Toggle (OnOff)"
        return (
            f"Input {self.input_number + 1} ({press_desc}): "
            f"{command_desc} ({cluster_name})"
        )


def get_preset_summary(raw_data: bytes | list[int]) -> str:
    """Parse raw InputActions data and return a human-readable summary.

    Args:
        raw_data: Raw bytes from InputActions attribute

    Returns:
        Multi-line string summary of actions
    """
    try:
        actions = InputActionsParser.parse(raw_data)
        if not actions:
            return "No actions configured."

        summary_lines = []
        # Group by input number for cleaner output
        actions.sort(key=lambda x: (x.input_number, x.press_type.value))

        for action in actions:
            summary_lines.append(f"- {action.to_friendly_string()}")

        return "\n".join(summary_lines)

    except Exception as err:
        return f"Failed to parse input actions: {err}"
