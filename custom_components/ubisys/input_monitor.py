...
async def async_setup_input_monitoring(
    ...
    for device in devices:
        ...
        existing_monitors = hass.data.get(DOMAIN, {}).get("input_monitors", [])
        if any(m.device_ieee == ieee for m in existing_monitors):
            _LOGGER.debug("Input monitor already exists for %s", ieee)
            continue
