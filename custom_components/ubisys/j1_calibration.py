...
1B. Prepare position (move down briefly to ensure not at top)
...
Phase 1B prevents "infinite loop at top" bug by ensuring shade is not at
the top limit before Phase 2 begins, per Official Step 4.
...
from .const import (
    ...
    UBISYS_ATTR_WINDOW_COVERING_TYPE,
...
async def _calibration_phase_1_enter_mode(
    ...
    - Step 2 (prepare limits) required per official procedure
    - No manufacturer code for Mode attribute writes
    ...
    try:
        await async_write_and_verify_attrs(
            cluster,
            {UBISYS_ATTR_WINDOW_COVERING_TYPE: window_covering_type},
            manufacturer=UBISYS_MANUFACTURER_CODE,
        )
    except Exception as err:
...
async def _calibration_phase_1b_prepare_position(
    ...
    This phase prevents the "infinite loop at top" bug.
...
async def _wait_for_motor_stop(
    ...
    Position monitoring (_wait_for_stall) doesn't work during calibration because
    position stays frozen at pre-calibration value, causing false "stall" detection
    after 3 seconds and stopping the motor before it reaches limits.
    ...
    See Also:
        ...
        - _wait_for_stall(): Position monitoring approach (doesn't work in
    ...
    while True:
        ...
        try:
            ...
            # Attribute read failed - log and retry with backoff
            read_failures += 1

            # Log transient failures as DEBUG (expected), only WARNING for final failure
            log_level = (
                logging.DEBUG if read_failures < MAX_READ_FAILURES else logging.WARNING
            )
            _LOGGER.log(
                log_level,
            ...
            # Exponential backoff: Give device time to recover after read failure
            # 1st failure: wait 1.0s, 2nd: 1.5s, 3rd: 2.0s, 4th: 2.5s
            backoff_delay = MOTOR_STATUS_POLL_INTERVAL + (read_failures * 0.5)
            _LOGGER.debug(
                "%s: Waiting %.1fs before retry (backoff after read failure)",
                phase_description,
                backoff_delay,
            )
            await asyncio.sleep(backoff_delay)
            continue  # Skip the normal poll interval since we already waited
...
async def _get_window_covering_cluster(
    ...
    try:
        ...
        # Probe EP1 first (per manual), then EP2 (fallback)
