...
Git hooks automatically enforce code quality before commits/pushes:

- **Pre-commit hook** (`.git/hooks/pre-commit`): Runs `make lint` before each commit
  - Black (formatting)
  - isort (import sorting)
  - flake8 (style guide)
  - Fast feedback (~5 seconds)

- **Pre-push hook** (`.git/hooks/pre-push`): Runs `make ci` before each push
  - All lint checks above
  - mypy (type checking)
  - pytest (tests with coverage)
  - Comprehensive validation (~15-30 seconds)
...
make lint       # Check formatting/style only
make typecheck  # Run mypy only
make test       # Run pytest only
make ci         # Full CI suite (recommended before committing)
```

**Why This Matters:**

Multiple consecutive releases can fail GitHub Actions CI if local checks aren't run. Git hooks prevent this by enforcing quality gates locally.
...
**Rationale**: Refactoring _perform_calibration from 274 lines to 81 lines by extracting phases improved:
...
- Using `add_config_entry` instead of `add_config_entry_id` in `device_registry.async_update_device()`
- Causes TypeError that breaks integration setup for all users
- Requires emergency hotfix release
...
For rapid iteration during active development, use the SSH-based deployment scripts:

```bash
# Full deployment: Sync files + restart Home Assistant Core
./scripts/deploy.sh

# Full deployment with log tailing
./scripts/deploy.sh --logs

# Quick sync only (manual reload required)
./scripts/sync.sh
```

**Benefits:**
- **45-90 seconds** per iteration (vs. 5-10 minutes for git release workflow)
- Bypasses file permission issues via `rsync --rsync-path="sudo rsync"`
- Automatically cleans Python cache (`__pycache__`, `*.pyc`)
- Ideal for hardware testing (calibration, input monitoring)
...
**See:** [docs/development_workflow.md](docs/development_workflow.md) for complete documentation
...
Incomplete local validation causes CI failures:
- Running only `mypy` gives false confidence
- Missing `black` (code formatter) causes CI formatting failure
- Missing `pytest` (test suite) causes test failures due to missing mocks
- Result: CI fails on GitHub Actions, requiring emergency fixes

**Root Cause**: Not running the full CI suite locally before committing.
