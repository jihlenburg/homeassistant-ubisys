...
def test_fmt_kv_sorts_keys():
    assert logtools._fmt_kv(beta=2, alpha=1) == "alpha=1, beta=2"


def test_info_banner_with_metadata(caplog: pytest.LogCaptureFixture):
...
def test_info_banner_without_metadata(caplog: pytest.LogCaptureFixture):
...
def test_kv_respects_log_level(caplog: pytest.LogCaptureFixture):
...
def test_kv_skips_when_disabled(caplog: pytest.LogCaptureFixture):
...
def test_stopwatch_elapsed_uses_monotonic_time():
